...
class FilterProdView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["brands"] = (
            Brands.objects.values("name").annotate(bcount=Count("products")).order_by()
        )
        context["cats"] = (
            Categories.objects.values("name")
            .annotate(ccount=Count("products"))
            .order_by()
        )
        context["disc"] = (
            Products.objects.values("discount")
            .annotate(dcount=Count("discount"))
            .order_by()
        )
        if self.request.GET:
